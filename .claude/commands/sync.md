...
또한 Codex hook surface인 `.codex/hooks.json`과 `.codex/config.toml`의 managed hook command set을 repo-root 기반 경로로 동기화한다.
...
**Step 1 — 인자 확인**
- 인수 `--dry-run` 또는 `dry-run`이면 dry-run 모드로 실행 (실제 복사 없이 변경 예정 보고)
- 인수 없으면 실제 동기화 실행
